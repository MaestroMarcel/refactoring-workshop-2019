      ...
      m_paused(false)
{
    ...
    if (w == 'W' and f == 'F' and s == 'S') {
        ...
        switch (d) {
            case 'U':
                snake.getCurrentDirection() = Direction_UP;
                break;
            case 'D':
                snake.getCurrentDirection() = Direction_DOWN;
                break;
            case 'L':
                snake.getCurrentDirection() = Direction_LEFT;
                break;
            case 'R':
                snake.getCurrentDirection() = Direction_RIGHT;
                break;
            default:
...
bool Controller::isSegmentAtPosition(int x, int y)
{
    return snake.getSegments().end() !=  std::find_if(snake.getSegments().cbegin(), snake.getSegments().cend(),
        [x, y](auto const& segment){ return segment.x == x and segment.y == y; });
}

bool Controller::isPositionOutsideMap(int x, int y)
{
...
Segment Controller::calculateNewHead()
{
    Segment const& currentHead = snake.getSegments().front();

    Segment newHead;
    newHead.x = currentHead.x + (isHorizontal(snake.getCurrentDirection()) ? isPositive(snake.getCurrentDirection()) ? 1 : -1 : 0);
    newHead.y = currentHead.y + (isVertical(snake.getCurrentDirection()) ? isPositive(snake.getCurrentDirection()) ? 1 : -1 : 0);
...
{
    auto tail = snake.getSegments().back();
...
{
    snake.getSegments().push_front(newHead);
...
void Controller::handleDirectionInd(std::unique_ptr<Event> e)
{
    ...
    if (perpendicular(snake.getCurrentDirection(), direction)) {
        snake.getCurrentDirection() = direction;
    }
}
...
std::list<Segment>& Snake::getSegments()
{
    return m_segments;
}
...
std::pair<int, int>& World::getMapDimension()
{
